...
- **본 실행 예상 비용**: 호출당 ≈1~2초 + sleep 1초. 2시장 × 5기간 전종목 등락률(10회, 각 KRX 요청 4회) + 지수 10회 + 유니버스·시총 ≈ 30회 → 약 1~2분.
  ...
  → **실측 (2026-09-08 `validate_main` 첫 실행, T=2026-09-07)**: `get_market_price_change` 1y 구간(2025-09-08 ~ 2026-09-07)
     **KOSPI 1y 4.08초 · KOSDAQ 1y 5.73초** (1d 2.05 / 1w 2.16 / 1m 2.44 / 6m 3.43초로 구간이 길수록 완만히 증가).
     전체 실행 2시장×5기간 **79.5초** (네트워크 호출 38회, 캐시 히트 20회, 재시도 0회). 지수 첫 호출 7.69초는 여전히 워밍업성 지연.
     → 본 실행 예산: 호출당 1~6초 + sleep 1초, 총 1.5~2분. GitHub Actions timeout 40분은 여유가 크다.
